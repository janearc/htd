...
module.exports = {
	...
	get: function (thing) {
		...
		if ( (typeof thing) === 'number' ) {
			...
			var signals = dg.deeply(
				signals,
				function (sig) {
					if (sig.numerics[ modefrob() ] === thing) {
						return sig;
					}
				},
				{
			...
			signals.forEach( function (sig) {
				var signame = (Object.keys( sig ))[0];
				candidates.push( new signal(
					sig[signame], sig.numerics[ modefrob() ], sig.action, sig.comment
				) );
			} );
			return candidates;
		}
			...
			else if (signals[ 'SIG'.concat( thing ) ]) {
				return new signal(
					sig[signame], sig.numerics[ modefrob() ], sig.action, sig.comment
				);
			}
